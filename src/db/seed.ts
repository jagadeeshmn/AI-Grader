...
import { assignments, usersSync } from "@/db/schema";
...
async function main() {
  try {
    ...
    console.log("🧹 Truncating assignments table and restarting identity...");
    ...
    await sql.query("TRUNCATE TABLE assignments RESTART IDENTITY CASCADE;");
    ...
    await seed(db, { assignments }, { seed: SEED }).refine((funcs) => ({
      assignments: {
        count: SEED_COUNT,
        columns: {
    ...
    console.log(`✅ Inserted ${SEED_COUNT} assignment(s) into the database\n`);

    // Ensure the assignments sequence is synced to the current MAX(id). This is a
    ...
    try {
      ...
      console.warn("⚠️ Failed to sync assignments sequence after seeding:", err);
    }
  } catch (err) {
